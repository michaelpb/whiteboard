...
function autogenerate_deck_from_dir(root_path) {
    ...
    for (let dir of directories) {
        ...
        slide.terminal = path.resolve(root_path, dir);

        if (dir === '.') {
            // First slide, adjust title
            slide.title = path_to_title(path.basename(root_path));
        }

        for (const file of files) {
            const fp = path.resolve(root_path, file);
