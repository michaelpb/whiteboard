...
class Terminal extends ModuleBase {
    ...
    getBinary() {
        const envVar = os.platform() === 'win32' ? 'COMSPEC' : 'SHELL';
        return process.env[envVar];
    }

    /*
    Try to guess a good set of args for the given binary
    */
    getArgs(binary) {
        if (os.platform() === 'darwin') {
            // For darwin, by default, /bin/bash doesn't include any rc files
            const rcFileGuess = pathlib.join(os.userInfo().homedir, '.bashrc');
            if (binary === '/bin/bash' && fs.existsSync(rcFileGuess)) {
                return ['--rcfile', rcFileGuess];
            }
        }
    ...
    spawn() {
        ...
        const args = this.getArgs(binary);
