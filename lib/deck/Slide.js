...
class Slide extends ModuleBase {
    ...
    static new_slide_info() {
        return {editpane:'title=New slide'};
    }

    // Given slide info object, lays out the panes.
    static layout_pane_previews(manager, slide_info) {
        const panes = [];
        for (const typename of Object.keys(slide_info)) {
            ...
            const preview = editor_class.get_iconic_preview
                ? editor_class.get_iconic_preview(slide_info[typename])
                : Slide.get_default_iconic_preview(typename);
            const hint = editor_class.layout_hint || {};
            panes.push({hint, preview});
        }
        const layout = slide_info.layout || HORIZONTAL_LAYOUT;
