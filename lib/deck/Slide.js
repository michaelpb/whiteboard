...
class Slide extends ModuleBase {
    ...
    static layoutRows(layout, panes, topPaneDefaultSize = TOP_PANE_SIZE) {
        ...
        // First remove nonpanular meta-data
        const realPanes = panes.filter(({ typename }) => !nonPanular.has(typename));

        // Next, float up all prefer_top
        const topPanes = realPanes.filter(pane => pane.hint.prefer_top);
        const normalPanes = realPanes.filter(pane => !pane.hint.prefer_top);
    ...
    */
    ...
    deletePane(typename) {
        ...
        this.manager.editors.callMethodForDescendants(editor, 'onWindowClosed');
        this.manager.editors.destroyEditor(editor);
